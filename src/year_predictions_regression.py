...
def main():
    fpath = '../notebooks/SongPredictions/'
    train = np.load(fpath+'years_train.npy')
    test  = np.load(fpath+'years_test.npy') 

    #X_tr   , y_tr    = transformed[:,:-1],transformed[:,-1]
    X_train_raw, y_train_raw = train[:,:-1], train[:,-1]
    X_test,  y_test   = test[:,:-1], test[:,-1]

    y_mean = np.mean(np.concatenate((y_train_raw, y_test),axis=0))
    for yy in [y_train_raw, y_test]:
        yy -= y_mean
        
    X_train, X_valid, y_train, y_valid = train_test_split(X_train_raw, y_train_raw, test_size=0.2,random_state=0)
    ...
    gammas = [10**_ for _ in np.arange(-5,5,step=0.25)]
    sketch_dimension = 256
    

    # Experimental setup
    num_trials = 5
    all_train_data = np.concatenate((X_train, X_valid),axis=0)
    ...
    for exp in range(num_trials):
        ...
        exact_results['test_error'][:,exp]   = exact_ridge.get_errors(X_test, y_test)
        ...
        rfd_results['test_error'][:,exp]   = rfd_ridge.get_errors(X_test, y_test)
        ...
        cl_results['test_error'][:,exp]   = cl_ridge.get_errors(X_test, y_test)
        ...
        hs_results['test_error'][:,exp]   = hs_ridge.get_errors(X_test, y_test)
...
def make_plots(gammas, exact_results, rfd_results, cl_results, hs_results):
    ...
    #plt.tight_layout()
    plt.show()  
    fig.savefig('experiments/figures/year_predictions.eps')
    fig.savefig('experiments/figures/year_predictions.pdf')
